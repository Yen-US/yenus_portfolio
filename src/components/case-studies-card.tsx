  ...
  return (
    <div className="flex h-full flex-col gap-8">
      <div className="flex items-baseline justify-between gap-3">
        <SectionLabel kicker="05" title="Signature case studies" />
        <p className="hidden font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground md:block">
          Microsoft for Startups · Presencia Studio
        </p>
      </div>

      <div className="flex flex-1 flex-col gap-10 divide-y divide-border/50">
        {caseStudies.map((cs, idx) => (
          <article
            key={cs.title}
            className={`flex flex-col gap-6 ${idx === 0 ? "" : "pt-10"}`}
          >
            {/* Metrics rail */}
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border/50 pt-4">
              ...
              {cs.stack.map((s, i) => (
                <span
                  key={s}
                  className="flex items-baseline gap-3 font-mono text-[11px] text-foreground/80"
                >
                  {s}
                  {i < cs.stack.length - 1 && (
